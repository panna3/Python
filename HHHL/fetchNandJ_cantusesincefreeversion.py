...
if not ALPHA_VANTAGE_API_KEY:
    raise ValueError("Please provide your Alpha Vantage API key in config.py")
...
# Function to fetch additional company details from Alpha Vantage
...
def filter_stocks(exchange, limit=SYMBOLS_LIMIT):
    filtered_stocks = []

    url = f'https://finnhub.io/api/v1/stock/symbol?exchange={exchange}&token={FINNHUB_API_KEY}'
    response = requests.get(url)
    print(f"Response status code for {exchange}: {response.status_code}")
    print(f"Response content for {exchange}: {response.content.decode('utf-8')}")

    if response.status_code != 200:
        print(f"Error fetching stock symbols for {exchange}")
        return []

    try:
        stocks = response.json()
        print(f"Parsed JSON type for {exchange}: {type(stocks)}")
        print(f"Parsed JSON for {exchange}: {stocks[:5]}")  # Print only the first 5 items for brevity
    except ValueError as e:
        print(f"Error parsing JSON for {exchange}: {e}")
        return []

    if not isinstance(stocks, list):
        print(f"Error: Expected a list of stocks for {exchange}")
        return []
    ...
    for i, stock in enumerate(stocks):
        ...
        # Fetch additional company details from Alpha Vantage
...
# Get the filtered list of stocks for NASDAQ and Japanese exchanges
filtered_stock_tickers = filter_stocks('NASDAQ') + filter_stocks('TSE')
...
for item in html_data:
    ...
    <div class="stock-item">
        <h2>{profile.get('name', 'N/A')} ({item['ticker']}) - {item['exchange']}</h2>
        <p><strong>Company:</strong> {profile.get('name', 'N/A')}</p>
        <p><strong>Sector:</strong> {profile.get('finnhubIndustry', 'N/A')}</p>
        <p><strong>Current Price:</strong> ${item['current_price']}</p>
        <p><strong>IPO Date:</strong> {profile.get('ipo', 'N/A')}</p>
        <p><strong>Website:</strong> <a href="{profile.get('weburl', '#')}" target="_blank">{profile.get('weburl', 'N/A')}</a></p>
